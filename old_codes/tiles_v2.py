...
def map_handler(string_map, character, w, h):
    """ ============================================== def map_x_handler ==============================================
    ...
    for column_index, column in enumerate(string_map):
        for row_index, row in enumerate(column):
            if row == character:
                horizontal.append(row_index * w)   # Onde está da <- p/ ->
                vertical.append(column_index * h)  # Onde está de cima p/ baixo
                x_found += 1  # Quantos
    # print(len(x_found), len(horizontal), len(vertical))
    return {
...
def insert_tile(
        group_box, tile_amount: int, where_x_at_horizontal: list, where_x_at_vertical: list, width_, height_
) -> None:
    """ =============================================== def insert_tile ===============================================
...
class Tile(pygame.sprite.Sprite):
    def __init__(self, picture_path, x, y, w, h):
        ...
        self.w = w
        self.h = h
        self.image = pygame.transform.scale(pygame.image.load(picture_path), (self.w, self.h)).convert_alpha()
...
map_setup = map_handler(string_map=scenario, character='X', w=32, h=32)
